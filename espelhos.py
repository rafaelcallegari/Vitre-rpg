...
def _aplicar_dano_no_jogador(jogador, dano):
    dano = max(1, int(dano))
    jogador.hp -= dano
    if jogador.hp <= 0:
        jogador.caiu = True
...
def _efeito_espelho_dardo_arcano(luta, jogador):
    ...
    dano = _aplicar_dano_no_jogador(jogador, _rolar_dano_espelho(luta, 2.0))
...
def _efeito_espelho_prisao_de_cristal(luta, jogador):
    ...
    dano = _aplicar_dano_no_jogador(jogador, at.aplicar_defesa(dano_bruto, jogador.s["def"]))
...
def _efeito_espelho_pancada_atordoante(luta, jogador):
    ...
    if random.random() < CHANCE_PANCADA_ESPELHO:
        condicoes.aplicar(luta, jogador.id, "pula_turno", dado["nome"], dado["emoji"], duracao=2, valor=0, origem="chefe")
        condicoes.aplicar(luta, jogador.id, "bloqueia_skill", dado["nome"], dado["emoji"], duracao=2, valor=0, origem="chefe")
        luta.registrar(f"{dado['emoji']} {luta.chefe['nome']} atordoa {jogador.nome}!")
    else:
        luta.registrar(f"{dado['emoji']} {luta.chefe['nome']} tenta atordoar {jogador.nome} e falha.")
...
def _efeito_espelho_voto_de_ferro(luta, jogador):
    ...
    aplica uma vez (`_voto_de_ferro_usado`), senão a rotina reaplicaria
    toda vez que "pressionar"/"reduzir_cura" batesse de novo. Ver
    decisoes.md."""
    dado = game_data.HABILIDADES["voto_de_ferro"]
    if luta.chefe.get("_voto_de_ferro_usado"):
        luta.registrar(f"{dado['emoji']} {luta.chefe['nome']} já jurou -- nada novo.")
        return
