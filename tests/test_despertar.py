...
async def _canal_fake(ctx):
    return None, False


def _deps():
    return {"dialogo_view_cls": _DialogoViewFake, "obter_canal_privado": _canal_fake}
...
async def _avancar_ate_pronome(user_id=1, classe_label="Guerreiro", oficio_label="Joalheiro"):
    ...
    ctx = _ctx(user_id)
    view1 = despertar.EscolhaClasseView(user_id, ctx, _deps())
...
def test_classe_e_oficio_ficam_so_na_view_ate_o_pronome():
    async def cenario():
        _, _, view3 = await _avancar_ate_pronome(classe_label="Mago", oficio_label="Encantador")
        return view3
...
def test_pronome_grava_classe_oficio_pronome_e_as_pocoes_juntos():
    async def cenario():
        ctx, it2, view3 = await _avancar_ate_pronome(classe_label="Guerreiro", oficio_label="Joalheiro")
...
def test_pronome_manda_a_fala_da_elna_com_a_concordancia_certa():
    async def cenario():
        ctx, it2, view3 = await _avancar_ate_pronome()
        it3 = _interacao(1, mensagem=it2.message)
        await _botao(view3, "ela / dela").callback(it3)
        return ctx
    ctx = asyncio.run(cenario())

    # duas mensagens novas depois do pronome: fala da Elna + fechamento
    embeds_enviados = [c.kwargs.get("embed") for c in ctx.send.call_args_list if c.kwargs.get("embed")]
...
def test_confirmacao_de_classe_usa_o_nome_certo_e_pronome_ainda_nao_escolhido():
    async def cenario():
        ctx = _ctx(1)
        view1 = despertar.EscolhaClasseView(1, ctx, _deps())
        it1 = _interacao(1)
...
def test_segunda_sequencia_nao_sobrescreve_quem_ja_terminou_a_primeira():
    async def cenario():
        _, it2a, view3a = await _avancar_ate_pronome(classe_label="Guerreiro", oficio_label="Joalheiro")
        it3a = _interacao(1, mensagem=it2a.message)
        await _botao(view3a, "ele / dele").callback(it3a)  # primeira sequência termina

        _, it2b, view3b = await _avancar_ate_pronome(classe_label="Mago", oficio_label="Alquimista")
