...
async def iniciar_raide(destino, ids, guilda_id, iniciador_id, editar=False):
    ...
    # Cooldown vem do tier da guilda (média do andar_max dos membros) --
    # tiers 0-1 ficam nas 2h de sempre, 2 cai pra 1h30, 3 pra 1h (nunca
    # menos, ver game_data.TIERS_GUILDA). Guilda com menos de 3 membros
    # fica no tier 0 qualquer que seja a média.
    cooldown = guildas.tier_da_guilda(guilda_id)[0]["cooldown_raide"]
...
def instalar(bot, contexto):
    ...
    print("raide.py carregado — rpg raide (mínimo 3, cooldown de 2h a 1h por guilda conforme o tier da guilda).")
